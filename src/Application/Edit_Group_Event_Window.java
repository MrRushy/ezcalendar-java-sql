...
public class Edit_Group_Event_Window {
	...
	private int groupEventIdToEdit;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Edit_Group_Event_Window window = new Edit_Group_Event_Window(1);
	...
	public Edit_Group_Event_Window(int groupEventId) { this.groupEventIdToEdit = groupEventId;
	initialize(); }
	...
	private void populateFields() {
		try {
			...
			String table = Ez_Calendar_Controller.currentGroupName.replace("_group", "_group_event");
			String query = "SELECT * FROM " + table + " WHERE group_event_id = ?";
			PreparedStatement stm = connection.prepareStatement(query);
			stm.setInt(1, groupEventIdToEdit);
		...
		public void saveChangesButton() {
			...
			addEventButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					System.out.println("[DEBUG] About to call editUserCalendarEvent with ID: " + groupEventIdToEdit);
					

					// Calls the 'editUserCalendarEvent' method from Ez_Calendar_Controller
					Ez_Calendar_Controller.groupLeaderEditGroupEvent(groupEventIdToEdit, eventTitle, eventDesc, eventDate, eventStart, eventEnd, eventLocation, originalTitle, originalDate);
					...
					View_Group_Calendar_Window PCW = new View_Group_Calendar_Window();	// Necessary for the following steps
